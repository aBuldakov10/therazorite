...
gulp.task('woff2', function () {
  return gulp.src(path.src.fonts_woff).pipe(gulp.dest('dest/fonts'));
});
...
gulp.task('build-clean', function () {
  return gulp.src('build').pipe(clean()); // Clean build directory
});

// Task for moving all HTML files from dest to build directory
gulp.task('build-html', function () {
  return gulp.src(path.build.html).pipe(gulp.dest('build'));
});
...
gulp.task('build-images', function () {
  return gulp.src(path.build.img).pipe(gulp.dest('build/img'));
});

// Task for moving all fonts from dest to build directory
gulp.task('build-fonts', function () {
  return gulp.src(path.build.fonts).pipe(gulp.dest('build/fonts'));
});
